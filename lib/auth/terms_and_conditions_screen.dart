...
class _TermsAndConditionsScreenState extends State<TermsAndConditionsScreen> {
  ...
  void initState() {
    super.initState();
    _loadTermsFromFirestore();
  }

  Future<void> _loadTermsFromFirestore() async {
    setState(() {
      _isLoading = true;
    });
    ...
    try {
      ...
      debugPrint("🔄 Fetching T&C - Auth user: ${user?.uid}");
      ...
      debugPrint("🔹 Retrieved ${snapshot.docs.length} T&C sections");

      if (!mounted) return;
      setState(() {
        _termsSections = snapshot.docs.map((doc) => doc.data()).toList();
      });
    } catch (e) {
        ...
        child: _isLoading
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ..._termsSections.map((section) => Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
  ...
  Widget _buildSectionContent(String text,
      {List<String>? bulletPoints, required double baseFontSize}) {
    return Padding(
